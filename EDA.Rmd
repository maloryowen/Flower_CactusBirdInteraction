...
```{r Site Wide Behavior Visuals only Stats}
#We can see that there are differences in how behavior was observed, but let's redo and only consider visuals.

#get counts for each behavior daily
daily.behav.vis <- (data.frame(table(site_visuals$behavior, site_visuals$date, site_visuals$walk)))

shapiro.test(daily.behav.vis$Freq)
#it's normally distributed

#run ANOVA 
dba.vis <- aov(Freq ~ Var1, daily.behav.vis)
summary(dba.vis)
#p-value < 2e-16, very significant (as expected)
...
#Dunnett Test
DunnettTest(formula = Freq ~ Var1, data = daily.behavior.vis)

#Tukey HSD
tuk2 <- TukeyHSD(dba, "Var1", ordered = TRUE)
tuktab2 <- data.frame(tuk2$Var1)
tuktab2 <- tuktab2 %>% rownames_to_column("behavior") 
tuktab2 <- transform(tuktab2, behavior = colsplit(behavior, split = "-", names = c('behavior1', 'behavior2'))) 
tuktab2$diff <- NULL 
tuktab2$lwr <- NULL 
tuktab2$upr<- NULL
