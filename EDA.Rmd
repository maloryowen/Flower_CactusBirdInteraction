...
>>>>>>> 615307322d01664e6ee99820bf1e60d543384d9a
...
#map the cacti in the survey's location
paired.fruit.flower.basemap <- get_map(location = c(lon = -115.662, lat = 34.7814), zoom = 19, color = "bw")

<<<<<<< HEAD
?get_map

allo.map <- ggmap(paired.fruit.flower.basemap)
allo.map <- allo.map +
  geom_point(data=allo, aes(x=lon, y=lat, colour = blooms, group = blooms), alpha = 3/10, size =4) +
  labs(title = "Cacti included in allocation survey", x = "longitude", y = "latitude", color = "Blooms Present (y/n") 
allo.map

#map based on plant volume compared to number of buds
allo.volume.buds <- ggmap(paired.fruit.flower.basemap)
allo.volume.buds <- allo.volume.buds +
  scale_color_gradient(low="#8CF554", high="#F59154") +
  geom_point(data = allo, aes(x = lon, y = lat, size = volume, color = buds), alpha = 8/10) 
allo.volume.buds
